...
// SCHEDULE GRAPH DATA RESET EVERY MORNING AT 7am
cron.schedule("0 7 * * *", async () => {
  try {
    // Delete or reset the model logic goes here
    // Example: Delete all documents from the Graph collection
    await Graph.deleteMany({});
    console.log("Model reset/delete completed.");
  } catch (err) {
    console.error("Error occurred during model reset/delete:", err);
  }
});
...
server.listen(PORT, () => {
  ...
  console.log(`Socket Server listening on ${process.env.PORT || 1028}`);
});
